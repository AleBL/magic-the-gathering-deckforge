...
import { FaCrown, FaShieldAlt, FaSync, FaPalette, FaTimes, FaExpand } from 'react-icons/fa';
...
function CardDetailModal({ card: initialCard, imageUrl, onAddToDeck, onClose, onSelectPrint }: CardDetailModalProps) {
  ...
  const { prints, isLoading: isPrintsLoading } = useCardPrints(card.name, card.oracle_id);
  ...
  return createPortal(
    <>
        ...
        >
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center md:items-start shrink-0 animate-fadeIn">
              ...
              {isPrintsLoading ? (
                <div className="flex flex-row md:flex-col gap-1.5 max-w-full max-h-20 md:max-h-[400px] py-1 shrink-0 animate-pulse select-none" aria-label="Loading editions">
                  {[1, 2, 3, 4].map((i) => (
                    <div
                      key={i}
      ...
      {selectedToken && (
        <div
          className="fixed inset-0 bg-black/85 flex items-center justify-center z-[60] p-4 animate-fadeIn"
          onClick={() => setSelectedToken(null)}
          onKeyDown={(e) => { if (e.key === 'Escape') setSelectedToken(null); }}
          role="button"
          tabIndex={-1}
          aria-label={t('close')}
        >
          ...
          >
            ...
            {(() => {
              const tokenImg =
                selectedToken.image_uris?.normal ||
                selectedToken.card_faces?.[0]?.image_uris?.normal ||
                '';
              return tokenImg ? (
                <img
                  src={tokenImg}
            ...
            <div className="text-center text-white space-y-1 animate-fadeIn">
              <h3 className="text-lg font-extrabold drop-shadow">
                {selectedToken.printed_name || selectedToken.name}
              </h3>
